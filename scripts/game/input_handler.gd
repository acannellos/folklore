...
func get_action(player: Entity) -> Action:
	...
	for direction in directions:
		if Input.is_action_just_pressed(direction) and not Global.death_flag and not Global.is_game_paused:
			var offset: Vector2i = directions[direction]
			action = BumpAction.new(player, offset.x, offset.y)
	
	if Input.is_action_just_pressed("wait") and not Global.is_game_paused:
